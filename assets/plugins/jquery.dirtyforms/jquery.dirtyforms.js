...
(function($){
	...
	$.extend({
		DirtyForms: {
			debug : false,
			message : 'Ci sono dei dati non ancora salvati. Si desidera continuare?',
			title : 'Sei sicuro che vuoi procedere?',
			dirtyClass : 'dirty',
			...
			dialog : {
				...
				bind : function(){
                    var close = function(decision) {
                        ...
                        $('*[id^="confirm-ok-"]').click(close(decidingContinue));
                        $('*[id^="confirm-cancel-"]').click(close(decidingCancel));
				},
				stash : function(){
                    return false;
				},
	...
	var methods = {
		init : function() {
			...
			return this.each(function(e){
				if (!$(this).is('form') && !$(this).is('div')) return;
		...
		setDirty : function() {
			...
			return this.each(function(e){
				$(this).addClass($.DirtyForms.dirtyClass).parents('div').addClass($.DirtyForms.dirtyClass);
                dirtylog($(this).addClass($.DirtyForms.dirtyClass).parents('div'));
			});
		},
	...
	var bindFn = function(ev){
		...
		if(($(ev.target).is('form') || $(ev.target).is('div')) && $(ev.target).parents(settings.dialog.selector).length > 0){
